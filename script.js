...
function checkExistingAuth() {
    ...
    if (token && user) {
        ...
        // Show navigation and go to appropriate dashboard
        showNavigationElements();
        updateUserInterface(currentUserType);
...
// Show navigation elements after successful login
function showNavigationElements() {
    ...
    if (currentUserSpan && currentUser) {
        currentUserSpan.textContent = currentUser.name || currentUser.userType;
    }
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Show user selection by default (unless already authenticated)
    if (!authToken) {
        document.getElementById('userSelection').classList.add('active');
    ...
    // Add authentication form event listeners
...
function toggleMobileNav() {
    ...
    if (navMenu.classList.contains('active')) {
        ...
        if (toggleBtnIcon) toggleBtnIcon.className = 'fas fa-bars';
    }
}
...
// Close mobile nav when a nav button is clicked (delegated)
...
function selectUserType(type) {
    ...
    // Hide entire header when going to auth page
    const header = document.querySelector('.header');
    if (header) header.style.display = 'none';
    
    // Update auth section titles based on user type
    const authTitle = document.getElementById('authTitle');
    const authSubtitle = document.getElementById('authSubtitle');
    
    if (type === 'vendor') {
        authTitle.textContent = 'Login as Vendor';
        authSubtitle.textContent = 'Access your dashboard and start sourcing materials';
    } else {
        authTitle.textContent = 'Login as Supplier';
        authSubtitle.textContent = 'Manage your inventory and connect with vendors';
    }
    
    // Show/hide conditional fields in register form
    ...
    if (type === 'vendor') {
        ...
        document.getElementById('gstNumber').required = false;
    } else {
        ...
        document.getElementById('gstNumber').required = true;
    }
...
// API Helper function
// Stub to avoid errors if something still calls apiCall; does nothing real.
async function apiCall(endpoint, method = 'GET', data = null) {
    console.warn('apiCall stub invoked; backend not configured.', endpoint, method, data);
    return {}; // harmless empty response
}
...
setInterval(() => {
    ...
    if (Math.random() > 0.8) {
        // Update some random order status
...
// ---- helper for showing messages above the auth forms ----
function showAuthMessage(msg, type = 'error') {
    let existing = document.getElementById('authMessage');
    if (existing) existing.remove();
    ...
    if (formContainer) formContainer.prepend(container);
}

// Simple in-browser user store
function getUsers() {
    try {
...
function handleRegister(e) {
    ...
    if (!name || !email || !password || !agree) {
        showAuthMessage('Please fill required fields and agree to terms.', 'error');
        return;
    }

    const users = getUsers();
    if (users.find(u => u.email === email)) {
        showAuthMessage('Email already registered. Please login.', 'error');
        return;
    }

    const userType = currentUserType || 'vendor';
    const newUser = {
        id: Date.now(),
        name,
        phone,
    ...
    authToken = 'demo-token-' + newUser.id;
    currentUser = newUser;
    currentUserType = newUser.userType;
    localStorage.setItem('authToken', authToken);
    localStorage.setItem('currentUser', JSON.stringify(currentUser));

    showNavigationElements();
    showSection(currentUserType === 'vendor' ? 'dashboard' : 'suppliers');
...
function handleLogin(e) {
    ...
    currentUser = user;
    currentUserType = user.userType || 'vendor';
    localStorage.setItem('authToken', authToken);
    localStorage.setItem('currentUser', JSON.stringify(currentUser));
...
// Override existing logout to clear demo session if needed
function logout() {
    localStorage.removeItem('authToken');
    localStorage.removeItem('currentUser');
    authToken = null;
